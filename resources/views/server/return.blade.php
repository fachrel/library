            ...
            <table class="table table-report -mt-2">
                ...
                <tbody>
                    @forelse ($loans as $loan)
                        <tr class="intro-x">
                            <td class="w-40 !py-4"> <a href="" class="underline decoration-dotted whitespace-nowrap">#{{ $loan->invoice }}</a> </td>
                            <td class="w-40">
                                <a href="" class="font-medium whitespace-nowrap">{{ $loan->user->name }}</a>
                                <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5">{{ $loan->user->address }}</div>
                            </td>
                            <td class="text-center">
                                <div class="flex items-center justify-center whitespace-nowrap
                                @if ($loan->CountReturnedBook() == 0)
                                    text-danger
                                @elseif($loan->CountReturnedBook() < $loan->CountBorrowedBook())
                                    text-warning
                                @else
                                    text-success
                                @endif
                            ...
                            <td class="table-report__action">
                                <div class="flex justify-center items-center">
                                    <a class="flex items-center text-primary whitespace-nowrap mr-5" href="{{ route('return.detail', $loan->id) }}"> <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" icon-name="check-square" data-lucide="check-square" class="lucide lucide-check-square w-4 h-4 mr-1"><polyline points="9 11 12 14 22 4"></polyline><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"></path></svg> Lihat Detail </a>
                                </div>
                            </td>
                        </tr>
                    @empty
